...
def KL_divergence(X, z):
    """
    Compute KL divergence between rows of X and z
    
    Args:
    - X: Matrix of reference distributions (rows)
    - z: Target distribution to compare against
    
    Returns:
    - Vector of KL divergence values
    """
    # Ensure non-negativity and sum to 1
    X_norm = X / (X.sum(dim=1, keepdim=True) + EPSILON)
    z_norm = z / (z.sum() + EPSILON)
    
    # Compute KL divergence
    KL_values = X_norm * (torch.log(X_norm + EPSILON) - torch.log(z_norm + EPSILON))
    return torch.sum(KL_values, dim=1)
...
if __name__ == "__main__":
	dataset = "mnist"
	# Load MNIST dataset
	batch_size = 500  # or another number you choose
	if dataset == "mnist":
		trainset, testset = load_mnist(batch_size)
	elif dataset == "cifar10":
		trainset, testset = get_cifar10(batch_size)
	

	# trainset[0][0] gives the images, trainset[0][1] would give the labels
	# imgs = trainset[0][0]  

	sim_fns = [dot_product_distance, manhatten_distance, cosine_similarity, KL_divergence]
	f = sim_fns[3] 	
	print(f.__name__)
	retrieval_results = []
	x = True
	for imgs, img_labels in trainset:
		N = batch_size
		num_plot = 0
		if x: 
			num_plot = 3
			x = False
		retrieval_result = PC_retrieve_store_continuous(
			imgs=imgs,  # input images
			N=N,  # number of images to process
			beta=1,
			# image_perturb_fn=gaussian_perturb_image,
			# sigma=0.1,  # perturbation level
			network_type="classical_hopfield",  # or another type you're using
			f=f,
			num_plot=num_plot,
			use_norm=False,
			ERROR_THRESHOLD=80
		...
		# break
		# The result will be the proportion of successfully reconstructed images
		print(f"\tReconstruction success rate: {retrieval_result}")

	print(retrieval_results)
	average = np.mean(retrieval_results)
	deviation = np.std(retrieval_results)
	retrieval = (average, deviation)
	print(retrieval)
	# break
	
	# for i in range(len(sim_fns)):
		# print(f"{sim_fns[i].__name__}: {retrievals[i]}")